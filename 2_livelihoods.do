...
egen check = rowtotal(HHTIncome CROPTOT1 LIVESTOCKTOT1 FISHINGTOT1 FOODSERVICETOT1 WHOLESALETOT1 MANUFACTURINGTOT1 TRANSPORTATIONTOT1 OTHERTOT1 OTHERPROG1 INCOMERECEIPTS1)
replace check=. if check==0
assert INCOMEALLSOURCES1==check
*/
...
// exclude own account activities to avoid double-counting
...
gen other_benefits = (	Q6_1_1A	== 1 | ///
						Q6_1_2A	== 1 | ///
						Q6_1_3A	== 1 | ///
						Q6_1_4A	== 1 | ///
						Q6_2_1A	== 1 )
					
gen other_other = (	Q6_3_1A	== 1 | ///
					Q6_3_2A	== 1 | ///
					Q6_3_3A	== 1 | ///
					Q6_4_1A	== 1 | ///
					Q6_4_2A	== 1 | ///
...
egen other_sources = rowmax (other_progs other_benefits other_other)

* number of income sources

egen n_sources = rowtotal(hh_n_jobs crop livestock fishing foodservice wholesale manufacturing trasportation other_activ other_sources)			
...
foreach x of varlist HHTIncome CROPTOT1 LIVESTOCKTOT1 FISHINGTOT1 FOODSERVICETOT1 WHOLESALETOT1 MANUFACTURINGTOT1 TRANSPORTATIONTOT1 OTHERTOT1 OTHERPROG1 INCOMERECEIPTS1 {
	gen share_`x' = `x'/INCOMEALLSOURCES1
	gen s2_`x' = (share_`x')^2
}

gen s_hh_vuln_income = hh_vuln_income/INCOMEALLSOURCES1

egen check_1 = rowtotal(share_*)
replace check_1 = round(check_1)
replace check_1=. if check_1==0
assert check_1==1 if !missing(check_1)

egen HHI = rowtotal(s2_*) // calculated manually, surely there's a stata command
